    ...
    for i in range(y_bounds[0], y_bounds[1]):
        for j in range(x_bounds[0], x_bounds[1]):
            if (i, j) in list_of_special_character_locations:
                gear_dict.setdefault((i, j), [])
                gear_dict[(i, j)].append(
                    contents[word_range[0]][word_range[1][0]:word_range[1][1]])
                return True
...
for i in gear_dict.keys():
    if len(gear_dict[i]) == 2:
        result += int(gear_dict[i][0]) * int(gear_dict[i][1])
